...
export function humanizeHrs(hours: number, decimalPlaces = 1) {
  ...
  if (years) {
    if (decimalPlaces) {
      const normYears = formatFloat(years + remainingYearHours / 8760, decimalPlaces);
  ...
  if (months) {
    if (decimalPlaces) {
      const normMonths = formatFloat(months + remainingMonthHours / 730, decimalPlaces);
  ...
  if (weeks) {
    if (decimalPlaces) {
      const normWeeks = formatFloat(weeks + remainingWeekHours / 168, decimalPlaces);
  ...
  if (decimalPlaces) {
    const normDays = formatFloat(days + remainingDayHours / 24, decimalPlaces);
