...
Call the obfuscate command with the `@spa-tools/runtime-config-obf` utility using
the domain-config input file path and the obfuscated output file path.

```bash
@spa-tools/runtime-config-obf obfuscate myapp-config.json myapp-config-obf.txt
```
...
file, you can call the deobfuscate command with the `@spa-tools/runtime-config-obf`
utility to recreate it.

```bash
@spa-tools/runtime-config-obf deobfuscate myapp-config-obf.txt myapp-config.json
```
